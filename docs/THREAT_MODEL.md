...
Status: Reactive-first engineering threat model. MARKOUT is not audited and is not approved for real funds.

## Scope and assets

The active scope is the Uniswap v4 hook, surcharge custody, trade lifecycle, Pyth publisher, Reactive Network pulse,
Reactive receiver, immutable settlement coordinator, and trader rebate claim path. The earlier Circle recovery
adapter, previous Reactive scheduler, and same-chain sampler remain historical or research artifacts rather than the
primary submission topology. Protected properties are:
...
| Historical Circle CCTP V2 module | Independently authenticated recovery research | Preserved for prior-phase evidence; cannot bypass hook validation or select a beneficiary |
| Pyth publisher caller | Fully untrusted | Supplies signed Pyth update bytes, exact fee, and trade id; cannot directly choose a price |
| Reactive system and callback proxy | Primary autonomous event-to-action transport | Observes the publisher event and must provide the immutable Reactive identity |
| Settlement coordinator | Immutable callback boundary | Sole hook settlement authority; a valid pending delivery can finalize once |
...
8. A Reactive callback or later authenticated duplicate cannot change a terminal trade allocation.
...
| Unauthorized settlement | Coordinator authorizes the immutable Reactive receiver; the receiver checks callback proxy plus ReactVM identity | Compromise of Pyth or Reactive's authenticated callback boundary remains an upstream risk |
| Replay or duplicate logs | Exact subscription, terminal hook state, and coordinator no-op semantics | Duplicate callbacks still consume delivery gas |
| Historical CCTP fast-confirmation reorg | Earlier recovery deployments used threshold `1000` to fit the testnet window | This residual risk applies to the historical recovery module, not the primary Reactive path |
...
| Reactive transport denial of service | Public expiry returns the full surcharge after the immutable grace period | An outage reduces LP protection and delays the rebate until expiry |
...
2. Reactive delivery may be retried; a callback against a terminal trade is harmless.
...
Operational monitoring should alert on aged pending trades, Reactive callback latency, rejected transport messages,
...
Pyth and Reactive trust boundaries, quote-basis, and delivery policy must receive independent review, supported tokens must be explicitly allowlisted,
